...
void main() {
  ...
  test('Should create a new product', () async {
    // Arrange
    final File? tImage =
        tProduct.imageUrl.isNotEmpty ? File(tProduct.imageUrl) : null;

    when(mockProductRepository.createProduct(tProduct, tImage))
        .thenAnswer((_) async => const Right(null));

    // Act
    final result =
        await createProductUsecase.call(const Params(product: tProduct));

    // Assert
    expect(result, const Right(null));
    verify(mockProductRepository.createProduct(tProduct, tImage));
